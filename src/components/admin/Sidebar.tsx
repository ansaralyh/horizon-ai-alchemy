...
const Sidebar: React.FC<SidebarProps> = ({ isOpen, setIsOpen }) => {
  ...
  return (
    <aside
      className={`fixed lg:static inset-y-0 left-0 z-50 flex flex-col transition-all duration-300 ease-in-out border-r border-white/5 bg-black/40 backdrop-blur-xl overflow-hidden ${
        isOpen ? "w-64 translate-x-0" : "w-0 -translate-x-full lg:w-20 lg:translate-x-0 lg:overflow-visible"
      }`}
    >
      <div className="p-4 sm:p-6 relative flex items-center justify-between lg:justify-center min-h-[4rem] shrink-0">
        {isOpen && (
          <Link to="/" className="flex items-center justify-center group w-full lg:w-auto">
            <img src={logo} alt="Logo" className="h-10 sm:h-12 lg:h-14 w-auto object-contain" />
          </Link>
        )}
        <button
        ...
        <button
          ...
          className="p-2 rounded-lg hover:bg-white/5 transition-colors text-muted-foreground lg:hidden ml-auto"
