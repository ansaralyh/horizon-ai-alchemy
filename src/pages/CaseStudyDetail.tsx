...
const CaseStudyDetail = () => {
  ...
  // Refined Dynamic Background Transitions
  // Bg 1 (Hero): Visible for Hero (S1) and Metrics (S2)
  const bg1Opacity = useTransform(scrollYProgress, [0, 0.2, 0.28], [1, 1, 0]);
  
  // Bg 2 (Content): Fades in for Challenge (S4) and Solution (S5)
  const bg2Opacity = useTransform(scrollYProgress, [0.35, 0.42, 0.65, 0.72], [0, 1, 1, 0]);
  
  // Bg 3 (Results/Impact): Optional for Results (S7)
  const bg3Opacity = useTransform(scrollYProgress, [0.75, 0.82], [0, 1]);
  ...
  const bgImages = {
    hero: caseStudy.id === 'mechanical-trading' ? "/portfilio001.png" : (caseStudy.image || "/assets/placeholder-hero.jpg"),
  ...
  return (
    <div className="min-h-screen bg-[#0B0F19] text-white selection:bg-amber-500/30 overflow-x-hidden">
      ...
      {/* --- PERSISTENT STICKY BACKGROUNDS --- */}
      <div className="fixed inset-0 z-0 pointer-events-none overflow-hidden">
        {/* Layer 1: Hero & Metrics Visibility */}
        <motion.div 
          style={{ opacity: bg1Opacity }}
          className="absolute inset-0 z-0"
        >
          <img src={bgImages.hero} alt="Hero Background" className="w-full h-full object-cover scale-105" />
          <div className="absolute inset-0 bg-black/60 backdrop-blur-[2px]" />
        </motion.div>

        {/* Layer 2: Core Project Content (Challenge/Solution) */}
        <motion.div 
          style={{ opacity: bg2Opacity }}
          className="absolute inset-0 z-0"
        >
          <img src={bgImages.content} alt="Content Background" className="w-full h-full object-cover scale-105" />
      ...
      <main className="relative z-10">
        ...
        <section className="section-with-bg section-bordered min-h-screen flex items-center justify-center pt-20">
          <div className="max-w-7xl mx-auto px-6 w-full text-center flex flex-col items-center">
             <motion.div {...fadeDown}>
             ...
             <div className="max-w-5xl space-y-8">
                ...
                {caseStudy.bonusLine && (
                  <motion.div 
                    ...
                    className={`py-4 border-l-4 ${theme.text.replace('text-', 'border-')} pl-6 bg-white/5 backdrop-blur-sm rounded-r-xl inline-block text-left mx-auto`}
                  >
                ...
                >
                  <a href="#challenge" className={`${theme.btn} px-10 py-5 text-base font-bold shadow-2xl`}>
                    Explore Case Study
                  </a>
                  <Link to="/contact" className={`${theme.btnOutline} px-10 py-5 text-base font-bold backdrop-blur-md`}>
                    Get in Touch
                  </Link>
                </motion.div>
             </div>
        ...
        <section className="section-with-bg section-bordered py-32 relative">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
              {caseStudy.hero.stats.map((stat, idx) => (
                <motion.div 
                  ...
                  className="metric-card group premium-blur"
                >
        ...
        {/* --- SECTION 3: OVERVIEW (CLEAN SOLID BG) --- */}
        {caseStudy.overview && (
          <section className="section-no-bg section-bordered py-32 relative">
             <div className="max-w-7xl mx-auto px-6 relative z-10">
                <div className="grid lg:grid-cols-12 gap-12 lg:gap-20 items-center">
                   <motion.div {...fadeLeft} className="lg:col-span-5 space-y-10">
                      ...
                      <div className="grid sm:grid-cols-2 gap-4">
                        {caseStudy.overview.features.map((feature, idx) => (
                          <div key={idx} className="flex items-center gap-3 p-4 rounded-xl bg-white/5 backdrop-blur-sm border border-white/10 hover:border-white/20 transition-colors group">
                   ...
                   <motion.div {...fadeRight} className="lg:col-span-7 relative group">
                      ...
                      <div className="relative rounded-3xl overflow-hidden border border-white/10 bg-[#0B0F19]/40 backdrop-blur-xl shadow-2xl transition-all duration-700 hover:border-white/20">
                         <img 
        ...
        {/* --- SECTION 4: CHALLENGE --- */}
        {caseStudy.challenge && (
          <section id="challenge" className="section-with-bg section-bordered py-32 relative overflow-hidden">
             <div className="max-w-7xl mx-auto px-6 relative z-10">
                <div className="grid lg:grid-cols-12 gap-12 lg:gap-20 items-center">
                   <motion.div {...fadeRight} className="lg:col-span-5 lg:order-2 space-y-10">
                      ...
                      <div className="space-y-4">
                        {caseStudy.challenge.problems.map((prob, idx) => (
                          <div key={idx} className={`flex items-start gap-4 p-5 rounded-2xl ${theme.bg} border ${theme.border} backdrop-blur-md hover:${theme.borderStrong} transition-all group`}>
                   ...
                   <motion.div {...fadeLeft} className="lg:col-span-7 lg:order-1 relative">
                      ...
                      <div className="relative rounded-[2.5rem] overflow-hidden border border-white/10 bg-[#0B0F19]/40 backdrop-blur-xl shadow-2xl group">
                ...
                <div className="grid lg:grid-cols-12 gap-12 lg:gap-20 items-center mb-24">
                   <motion.div {...fadeLeft} className="lg:col-span-6 space-y-10">
                      <div className="space-y-6">
                        ...
                        <p className={`text-xl text-[#A0AEC0] leading-premium border-l-4 ${theme.text.replace('text-', 'border-')}/50 pl-8 premium-blur py-4 pr-4 rounded-r-xl`}>
                         ...
                         {caseStudy.featuresList.map((feat, idx) => {
                            ...
                            return (
                              <motion.div 
                                ...
                                {...fadeUp}
                                transition={{ duration: 0.5, delay: idx * 0.1 }}
                                className="group p-10 rounded-[2.5rem] premium-blur transition-all duration-500 relative overflow-hidden"
                              >
        ...
        {/* --- SECTION 7: RESULTS (SOLID BG) --- */}
        {(caseStudy.results || caseStudy.performance) && (
          <section id="results" className="section-no-bg section-bordered py-32 relative overflow-hidden">
             <div className="max-w-7xl mx-auto px-6 relative z-10">
                <div className="grid lg:grid-cols-12 gap-12 lg:gap-20 items-center">
                   <motion.div {...fadeLeft} className="lg:col-span-5 space-y-10">
                      <div className="space-y-6">
                        ...
                        <p className={`text-xl text-[#A0AEC0] leading-premium border-l-4 ${theme.text.replace('text-', 'border-')}/50 pl-8 bg-white/5 backdrop-blur-sm py-4 rounded-r-xl`}>
                      ...
                      <div className="grid gap-6">
                        {(caseStudy.results?.bullets || caseStudy.performance?.bullets || []).map((bullet, idx) => (
                           <div key={idx} className="flex items-center gap-5 group p-4 rounded-2xl bg-white/5 backdrop-blur-md border border-white/5 hover:bg-white/[0.08] transition-all">
                   ...
                   <motion.div {...fadeRight} className="lg:col-span-7 relative group">
                      <div className={`absolute -inset-10 ${theme.bg} rounded-full blur-[120px] opacity-20 group-hover:opacity-30 transition-opacity`} />
                      <div className="relative rounded-2xl md:rounded-[2.5rem] overflow-hidden border border-white/10 bg-[#0B0F19]/40 backdrop-blur-xl shadow-2xl transition-all duration-500 hover:border-white/20">
                         <img 
        ...
        {/* --- SECTION 8: TECH STACK & SKILLS (SOLID BG) --- */}
                     ...
                     <motion.div {...fadeRight} className="space-y-10">
                        ...
                        <div className="flex flex-wrap gap-4">
                           {caseStudy.skills.map((skill, idx) => (
                             <div key={idx} className={`px-6 py-3 rounded-xl border border-white/10 hover:border-white/30 transition-all text-gray-400 hover:text-white font-medium bg-navy-card/50`}>
                                {skill}
                             </div>
                           ))}
                        </div>
                      ...
                      <div className="grid sm:grid-cols-2 gap-6">
                        {caseStudy.monthly.returns.map((ret, idx) => (
                          <motion.div 
                            ...
                            {...fadeUp}
                            transition={{ duration: 0.5, delay: idx * 0.1 }}
                            className={`p-8 rounded-3xl bg-navy-card/80 backdrop-blur-md border border-white/10 flex flex-col gap-4 group hover:${theme.border} transition-all duration-300`}
                          >
                ...
                <div className="grid lg:grid-cols-2 gap-20">
                   <motion.div {...fadeLeft} className="space-y-6">
                      ...
                      <div className="grid sm:grid-cols-2 gap-6 pt-8">
                         <div className="p-6 rounded-2xl bg-white/5 backdrop-blur-sm border border-white/5 space-y-4">
                   ...
                   <div className="space-y-6">
                      {caseStudy.risk.metrics.map((m, idx) => (
                        <motion.div 
                          ...
                          {...fadeRight}
                          transition={{ duration: 0.5, delay: idx * 0.1 }}
                          className={`flex justify-between items-center p-8 rounded-3xl bg-navy-card/80 backdrop-blur-md border border-white/10 group hover:${theme.borderStrong} transition-all`}
                        >
        ...
        {caseStudy.trust && (
          <section className="section-no-bg section-bordered py-40 relative">
                ...
                <div className="flex flex-wrap justify-center gap-6">
                   {caseStudy.trust.points.map((p, idx) => (
                    <motion.div 
                      ...
                      {...fadeUp}
                      transition={{ duration: 0.5, delay: idx * 0.1 }}
                      className={`flex items-center gap-4 px-8 py-5 bg-navy-card/80 backdrop-blur-md border border-white/10 rounded-2xl group hover:${theme.borderStrong} transition-all`}
                    >
        ...
        {/* --- SECTION 12: CTA --- */}
        {caseStudy.cta && (
          <section className="section-no-bg py-40 px-6">
             <motion.div 
               {...fadeUp}
               className={`max-w-7xl mx-auto rounded-[4rem] px-8 py-20 relative overflow-hidden text-center space-y-8 bg-gradient-to-b from-[#111726] to-[#0B0F19] border border-white/5 shadow-2xl`}
             >
                ...
                <div className="flex flex-col sm:flex-row justify-center gap-6 pt-6">
                  {caseStudy.cta.buttons.map((btn, idx) => (
                    <Link key={idx} to={btn.href} className={`${idx === 0 ? theme.btn : theme.btnOutline} px-10 py-5 text-lg transition-all hover:scale-105 active:scale-95 backdrop-blur-md`}>
                      {btn.label}
                    </Link>
                  ))}
                </div>
